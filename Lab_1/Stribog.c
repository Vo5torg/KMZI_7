...
static void Stribog_calc_chunk(struct Stribog_buff* buff, const uint8_t* chunk) {
	uint32_t w[64];
	uint32_t tv[8];
	uint32_t i;
...
void Stribog_update(struct Stribog_buff* buff, const void* data, size_t size) {
	...
	if (size + buff->chunk_size >= 64) {
		...
		buff->chunk_size = 0;
		Stribog_calc_chunk(buff, tmp_chunk);
	}
	/* Run over data chunks */
	while (size >= 64) {
		Stribog_calc_chunk(buff, ptr);
		ptr += 64;
		size -= 64;
	}
...
void Stribog_finalize(struct Stribog_buff* buff) {
	...
	if (buff->chunk_size > 56) {
		Stribog_calc_chunk(buff, buff->last_chunk);
...
void Stribog_easy_hash_hex(const void* data, size_t size, char* hex) {
	uint8_t hash[32];
	Stribog_easy_hash(data, size, hash);
	bin_to_hex(hash, 32, hex);
}